...
for k = 1:runs
    ...
    psource = 0; %Not using a point source
...
% xlim([-6 6]); ylim([-6 6]);
    ...
    [~,ix] = min(abs(x_det_points - g.path(lor1(end),1))); 
    [~,ix2] = min(abs(x_det_points - g.path(lor2(end),1)));
    ...
    for j = 1:length(myGrid.y2)
        for i = 1:length(myGrid.x2)
            if isempty(find(LOR(:,1) > myGrid.x2(i,1) & g.path(:,1) <=...
